...
def get_primary_profile(db: Session, user_id: int) -> Result[Profile]:
    with log.time(f"get_primary_profile: user_id={user_id} {{elapsed}}"):
...
def save_profile_draft(
    ...
    *,
    ...
    with log.time(f"save_profile_draft: profile_id={profile_id} {{elapsed}}"):
...
def get_my_latest_draft(db: Session, *, profile_id: int, editor_user_id: int) -> Result[ProfileDraftResponse]:
    with log.time(f"get_my_latest_draft: profile_id={profile_id} {{elapsed}}"):
        draft = db.scalar(
...
def get_public_profile(db: Session, *, profile_id: int) -> Result[PublicProfile]:
    with log.time(f"get_public_profile: profile_id={profile_id} {{elapsed}}"):
        row = db.execute(
            select(Profile, User)
